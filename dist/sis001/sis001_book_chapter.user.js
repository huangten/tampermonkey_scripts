...
(function() {
	...
	function run() {
		layui.use(function() {
			layui.util.fixbar({
				...
				click: function(type) {
					if (type === "复制内容") getContent();
					if (type === "复制内容HTML") getContentHtml();
					if (type === "复制内容（第二版）") getContentV2();
					if (type === "复制内容HTML（第二版）") getContentHtmlV2();
				}
			});
		});
	}
	function getElement(uw) {
		let e1 = uw.document.getElementsByClassName("postcontent")[0].getElementsByClassName("postmessage")[0].cloneNode(true);
		let fieldset = e1.getElementsByTagName("fieldset")[0];
		let table = e1.getElementsByTagName("table")[0];
		e1.querySelectorAll("*").forEach((element) => {
			if (element === fieldset) element.remove();
			if (element === table) element.remove();
	...
	function getContentV2(uw = _unsafeWindow) {
		copyContext(getElement(uw).innerText.replaceAll("\n", "").replaceAll("	", "\n").replaceAll("　　", "\n　　").split("\n").filter(Boolean).join("\n")).then();
	}
