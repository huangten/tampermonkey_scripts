		...
		layui.use(function() {
			layui.util.fixbar({
				...
				click: function(type) {
					...
					if (type === "下载信息和种子") getInfo(document);
					if (type === "仅复制标题") copyContext(getTitleText(document).trim()).then();
					if (type === "复制标题和下载种子") copyTitleAndDownload(document);
					if (type === "复制标题和磁力信息") copyTitleAndBlockcode(document);
				}
			});
		});
	}
