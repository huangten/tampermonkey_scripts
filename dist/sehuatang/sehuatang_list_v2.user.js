...
(function(file_saver) {
	...
	function getDownloadBtTags(el) {
		...
		let aTags = [];
		if (attnms !== null && attnms.length === 0) aTags = el.getElementsByClassName("t_fsz")[0].getElementsByTagName("table")[0].getElementsByTagName("tr")[0].getElementsByTagName("a");
	...
	function doBtDownload(el) {
		let attnms = getDownloadBtTags(el);
		for (let index = 0; index < attnms.length; index++) downloadBTFile(attnms[index].href, attnms[index].innerText.trim());
	}
	...
	function getTitle(el) {
		if (el.getElementById !== void 0) return el.getElementById("thread_subject");
	...
	var downloadWindowId = 0;
	var divId = "downloadWindowDivId";
...
<div class="layui-progress layui-progress-big" lay-showPercent="true" lay-filter="demo-filter-progress"> <div class="layui-progress-bar layui-bg-orange" lay-percent="0%"></div></div>  </div></fieldset><div id="${divId}" style="width: 100%;height: 100%;"></div>`,
	...
	async function ensureDownloadWindow(divId = "downloadWindowDivId") {
		if (downloadWindowId !== 0) return downloadWindowId;
	...
	function run() {
		document.onvisibilitychange = () => {
			if (document.visibilityState === "visible" && document.readyState === "complete") check18R();
		};
		setTimeout(() => {
			check18R();
		}, 500);
		if (document.location.href === "https://sehuatang.org/forum.php") return;
		...
		layui.use(function() {
			layui.util.fixbar({
				...
				margin: 0,
				click: function(type) {
					if (type === "downloadAll") downloadAll();
					if (type === "clearDownloadList") downloader.clear();
					if (type === "menuList") openMenuPage();
				}
			});
		});
	}
		...
		layui.layer.open({
			...
			success: function(layero, index, that) {
				...
				util.fixbar({
					...
					bgcolor: "#ba350f",
					click: function(type) {
						if (type === "getCheckedNodeData") treeCheckedDownload();
						if (type === "clear") reloadTree();
					}
				});
				tree.render({
					elem: "#openPage",
				...
				function reloadTree() {
					tree.reload("titleList", { data: getTree() });
					downloader.clear();
				}
			}
		});
