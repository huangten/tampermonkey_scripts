    ...
    async waitForChapterLoad(iframe, chapter) {
      await new Promise((resolve, reject) => {
        ...
        iframe.onload = async () => {
          try {
            this.assertChapterDocumentHealth(iframe.contentDocument, chapter);
            await waitForElement(iframe.contentDocument, ".reader-content", 1e3 * 25 * 60);
            clearTimeout(timeout);
            resolve();
          } catch (err) {
            clearTimeout(timeout);
...
page: false,
    ...
    async exportChaptersJson() {
      ...
      try {
        ...
        fileSaver.saveAs(blob, this.getChaptersJsonExportFileName());
