...
(function() {
	...
	function run() {
		layui.use(function() {
			layui.util.fixbar({
				...
				click: function(type) {
					if (type === "复制书名") getBookname();
					if (type === "复制内容") getPreTagContent();
					if (type === "原样下载") downloadChapterContent();
					if (type === "添加空白符下载") downloadChapterContent("blank");
					if (type === "复制内容HTML") getPreTagContentHtml();
					if (type === "调整排版并复制") copyChapterContent();
				}
			});
		});
	}
	...
	function getBookname() {
		const titleContent = document.getElementsByClassName("main-title")[0].innerText.trim();
	...
	function downloadChapterContent(tag) {
		let title = document.getElementsByClassName("main-title")[0].innerText.trim().replace(/^【(.*?)】/, "$1");
